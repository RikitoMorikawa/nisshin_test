...
import {
  ...
  SmallCategory,
} from 'src/app/models/small-category';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof SmallCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'medium_category_name', name_jp: '中分類名' },
    { name: 'name', name_jp: '小分類名' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.smService.getAll(this.createApiParams(x))),
  ...
  private createApiParams(arg: object) {
    ...
    return {
      ...
      medium_category_id: this.selectedLcId ? this.selectedLcId : '',
    };
  }
  ...
  private createTableParams(
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/small-category/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: '小分類一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
