...
} from 'src/app/models/custom-medium-category';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof CustomMediumCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'custom_large_category_name', name_jp: 'カスタム大分類名' },
    { name: 'name', name_jp: 'カスタム中分類名' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.cmcService.getAll(this.createApiParams(x))),
  ...
  private createApiParams(arg: object) {
    ...
    return {
      ...
      custom_large_category_id: this.selectedLcId ? this.selectedLcId : '',
    };
  }
  ...
  private createTableParams(
    res: CustomMediumCategoryApiResponse
  ): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/custom-medium-category/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'カスタム中分類一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
