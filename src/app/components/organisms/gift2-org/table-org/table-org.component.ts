...
import { Gift2, Gift2ApiResponse } from 'src/app/models/gift2';
...
export class TableOrgComponent implements OnInit {
  // テーブルイベント購読用
  ...
  // APIレスポンスとヘッダ名のマッピング
  tableNameMapping: { name: keyof Gift2; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: 'ギフト2名' },
    { name: 'furi', name_jp: 'ギフト2名ふりがな' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.gift2Service.getAll(this.createApiParams(x))),
   ...
   */
  createTablePrams(res: Gift2ApiResponse): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/non-cash-item/gift2/detail/${row[column]}`,
        text: row[column] + '',
      };
      // 登録日時
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'ギフト2一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
