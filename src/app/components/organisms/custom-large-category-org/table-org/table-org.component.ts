...
import {
  ...
  CustomLargeCategory,
} from 'src/app/models/custom-large-category';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof CustomLargeCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: 'カスタム大分類名' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.clcService.getAll(this.createApiParams(x))),
  ...
  private createTableParams(
    res: CustomLargeCategoryApiResponse
  ): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/custom-large-category/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'カスタム大分類一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
