...
import { Role, RoleApiResponse } from 'src/app/models/role';
import { RoleService } from 'src/app/services/role.service';
import { modalConst } from 'src/app/const/modal.const';
import { CommonService } from 'src/app/services/shared/common.service';

@Component({
  selector: 'app-role-table-org',
  templateUrl: './role-table-org.component.html',
  styleUrls: ['./role-table-org.component.scss'],
})
export class RoleTableOrgComponent implements OnInit, OnDestroy {
  constructor(private roleService: RoleService, public common: CommonService) {}
  ...
  tableNameMapping: { name: keyof Role; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: '権限名' },
    { name: 'description', name_jp: '権限概要' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.roleService.getAll(this.createApiParams(x))),
   ...
   */
  private createTableParams(res: RoleApiResponse): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/role/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: '権限一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
