...
import {
  ...
  LargeCategory,
} from 'src/app/models/large-category';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof LargeCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: '大分類名' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.lcService.getAll(this.createApiParams(x))),
  ...
  private createTableParams(
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/large-category/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: '大分類一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
