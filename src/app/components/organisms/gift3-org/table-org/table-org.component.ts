...
import { Gift3ApiResponse, Gift3 } from 'src/app/models/gift3';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof Gift3; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: 'ギフト3名' },
    { name: 'furi', name_jp: 'ギフト3名カナ' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.gift3Service.getAll(this.createApiParams(x))),
   ...
   */
  createTablePrams(res: Gift3ApiResponse): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/non-cash-item/gift3/detail/${row[column]}`,
        text: row[column] + '',
      };
      // 登録日時
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'ギフト3一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
