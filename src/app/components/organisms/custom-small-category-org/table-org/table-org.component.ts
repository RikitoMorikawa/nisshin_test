...
import {
  ...
  CustomSmallCategory,
} from 'src/app/models/custom-small-category';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  constructor(
    private csmService: CustomSmallCategoryService,
  ...
  tableNameMapping: { name: keyof CustomSmallCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'custom_medium_category_name', name_jp: 'カスタム中分類名' },
    { name: 'name', name_jp: 'カスタム小分類名' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.csmService.getAll(this.createApiParams(x))),
  ...
  private createApiParams(arg: object) {
    ...
    return {
      ...
      custom_medium_category_id: this.selectedLcId ? this.selectedLcId : '',
    };
  }
  ...
  private createTableParams(
    res: CustomSmallCategoryApiResponse
  ): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/custom-small-category/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'カスタム小分類一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
