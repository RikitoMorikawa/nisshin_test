...
import {
  ...
  MediumCategory,
} from 'src/app/models/medium-category';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof MediumCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'large_category_name', name_jp: '大分類名' },
    { name: 'name', name_jp: '中分類名' },
    { name: 'short_name', name_jp: '短縮名' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.mcService.getAll(this.createApiParams(x))),
  ...
  private createApiParams(arg: object) {
    ...
    return {
      ...
      large_category_id: this.selectedLcId ? this.selectedLcId : '',
    };
  }
  ...
  private createTableParams(
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/medium-category/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: '中分類一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
