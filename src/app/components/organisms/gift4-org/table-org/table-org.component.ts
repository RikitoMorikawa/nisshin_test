...
import { Gift4ApiResponse, Gift4 } from 'src/app/models/gift4';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof Gift4; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: 'ギフト4名' },
    { name: 'furi', name_jp: 'ギフト4名カナ' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.gift4Service.getAll(this.createApiParams(x))),
   ...
   */
  createTablePrams(res: Gift4ApiResponse): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/non-cash-item/gift4/detail/${row[column]}`,
        text: row[column] + '',
      };
      // 登録日時
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'ギフト4一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
