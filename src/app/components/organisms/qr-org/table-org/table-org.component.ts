...
import { QrApiResponse, Qr } from 'src/app/models/qr';
...
export class TableOrgComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof Qr; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: 'QR名' },
    { name: 'furi', name_jp: 'QR名カナ' },
   ...
   * @param qrService
   */
  constructor(private qrService: QrService, public common: CommonService) {}
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.qrService.getAll(this.createApiParams(x))),
   ...
   */
  createTablePrams(res: QrApiResponse): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/non-cash-item/qr/detail/${row[column]}`,
        text: row[column] + '',
      };
      // 登録日時
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'QR一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
