...
export class AddComponent implements OnInit, OnDestroy {
  ...
  form = this.fb.group({
    ...
    payment_detail: this.fb.array([
      this.fb.group({
        id: [''],
        payment_id: [''],
        payment_supplier_id: ['', [Validators.required]],
        payment_supplier_name: [''],
        scheduled_payment_amount: [''],
        payment_type_division_code: ['', [Validators.required]],
        payment_amount: [''],
        payment_date: [''],
        remarks_1: [''],
      }),
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // console.log(res);
            // console.log(`223 ${this.detailPagePath}`);
  ...
  initOptions() {
    ...
    // 日付のサジェスト
    this.ctrls.payment_date.valueChanges.subscribe(() => {
      this.payment_date = this.ctrls.payment_date.value
        ? new Date(this.ctrls.payment_date.value).toLocaleDateString()
        : '';
    });
  ...
  handleClickCancel(): void {
    ...
    if (!this.form.pristine) {
      ...
      // console.log(`325 ${this.detailPagePath}`);
      this.router.navigateByUrl(this.detailPagePath);
    }
  }
  ...
  handleClickSaveButton() {
    ...
    rawDelValues.forEach((x) => {
      const paymentDetails = {
        ...
        payment_supplier_id: Number(x.payment_supplier_id),
        payment_supplier_name: x.payment_supplier_name,
        payment_type_division_code: x.payment_type_division_code,
        //estimated_payment_amount: x.estimated_payment_amount,
      ...
      suppliers.push(Number(x.payment_supplier_id));
    });

    let suppliers_liist: any = this.uniq(suppliers);
    let observable$ = [];

    // supplier_id 毎に保存処理
    for (let index in suppliers_liist) {
      const detail_list = this.paymentDetailAny.filter((x: any) => {
        return suppliers_liist[index] == x.payment_supplier_id;
      });
      const postData: paymentAddDetailDataType = {
        supplier_id: Number(detail_list[0].payment_supplier_id),
        created_id: Number(this.author.id),
        remarks_1: formVal.remarks_1 || '',
        payment_detail: [...detail_list],
      };

      observable$.push(this.paymentService.add(postData));
    }
    // 格納処理
    forkJoin(observable$)
      .pipe(
  ...
  calc() {
    ...
    return this.supplier.account_payable;
  }
