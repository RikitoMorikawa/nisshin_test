...
import {
  PbSecondCategory,
  PbSecondCategoryApiResponse,
} from 'src/app/models/pb-second-category';
...
export class TableComponent implements OnInit, OnDestroy {
  ...
  @Input() selectedLcId?: number;
  @Input() selectedLcName?: string;
  ...
  tableNameMapping: { name: keyof PbSecondCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'product_book_first_category_name', name_jp: '第1分類名' },
    { name: 'name', name_jp: '第2分類名' },
    { name: 'furi', name_jp: 'フリガナ' },
    { name: 'book_cd', name_jp: 'ブックコード' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) =>
            this.pbSecondCategoryService.getAll(this.createApiParams(x))
          ),
  ...
  private createApiParams(arg: object) {
    ...
    return {
      ...
      product_book_first_category_id: this.selectedLcId
        ? this.selectedLcId
        : '',
    };
  }
  ...
  private createTableParams(
    res: PbSecondCategoryApiResponse
  ): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/product-book-second-category/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: '商品ブック第2分類一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
