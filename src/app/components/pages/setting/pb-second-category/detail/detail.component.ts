...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = '商品ブック第2分類詳細：' + modalConst.TITLE.HAS_ERROR;

  // 編集画面へのパスを保持
  editPath!: string;

  // 商品ブック第2分類一覧のパス
  pbSecondCategoryListPath = '/setting/product-book-second-category';

  // メイン商品ブックの一覧のパス
  mainProductBookPath = '/setting/main-product-book';
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // 商品ブック第2分類詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.pbSecondCategoryListPath);
          }
        })
    );
    ...
    this.editPath =
      '/setting/product-book-second-category/edit/' + this.selectedId;
    ...
    this.subscription.add(
      this.pbSecondCategoryService
        ...
        .subscribe((res) => {
          ...
          this.pbSecondCategory = displayData;
          if (displayData.image_path) {
            this.logo = displayData.image_path;
          }
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = '商品ブック第2分類：' + modalConst.TITLE.DELETE;
              ...
              .subscribe((res) => {
                ...
                if (res instanceof HttpErrorResponse) {
                  ...
                  this.errorService.setError({
                    ...
                    redirectPath: this.pbSecondCategoryListPath,
                  });
                } else {
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '商品ブック第2分類詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.pbSecondCategoryListPath,
    });
  }
