...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = '小分類詳細：' + modalConst.TITLE.HAS_ERROR;
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // 小分類詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.smallCategoryPath);
          }
        })
    );
    ...
    this.editPath = '/setting/small-category/edit/' + this.selectedId;
    ...
    this.subscription.add(
      this.smService
        ...
        .subscribe((res) => {
          ...
          this.smallCategory = displayData;
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = '小分類：' + modalConst.TITLE.DELETE;
              ...
              .subscribe((res) => {
                ...
                if (res instanceof HttpErrorResponse) {
                  ...
                  this.errorService.setError({
                    ...
                    redirectPath: this.smallCategoryPath,
                  });
                } else {
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '小分類詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.smallCategoryPath,
    });
  }
