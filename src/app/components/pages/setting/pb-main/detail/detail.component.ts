...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = 'メイン商品ブック詳細：' + modalConst.TITLE.HAS_ERROR;
  ...
  mainProductBook!: MainProductBook;
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // メイン商品ブック詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.mainProductBookPath);
          }
        })
    );
    ...
    this.editPath = '/setting/main-product-book/edit/' + this.selectedId;
    ...
    this.subscription.add(
      this.pbMainService
        ...
        .subscribe((res) => {
          ...
          this.mainProductBook = displayData;
          this.add_user =
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = 'メイン商品ブック：' + modalConst.TITLE.DELETE;
              ...
              .subscribe((res) => {
                ...
                if (res instanceof HttpErrorResponse) {
                  ...
                  this.errorService.setError({
                    ...
                    redirectPath: this.mainProductBookPath,
                  });
                } else {
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'メイン商品ブック：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.mainProductBookPath,
    });
  }
