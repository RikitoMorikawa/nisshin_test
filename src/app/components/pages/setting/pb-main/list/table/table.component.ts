...
import {
  MainProductBook,
  MainProductBookApiResponse,
} from 'src/app/models/pb-main';
...
export class TableComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof MainProductBook; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'product_book_second_category_name', name_jp: '第2分類名' },
    { name: 'book_cd', name_jp: 'ブックコード' },
    { name: 'product_name', name_jp: '商品名' },
    { name: 'gyo', name_jp: 'プリントコード' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) => this.pbMainService.getAll(this.createApiParams(x))),
  ...
  private createApiParams(arg: object) {
    ...
    return {
      ...
      product_book_second_category_id: this.selectedLcId
        ? this.selectedLcId
        : '',
    };
  }
  ...
  private createTableParams(
    res: MainProductBookApiResponse
  ): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/main-product-book/detail/${row[column]}`,
        text: row[column] + '',
      };
  ...
  handleError(status: number, message: string) {
    this.errorEvent.emit({
      status: status,
      title: 'メイン商品ブック一覧：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
