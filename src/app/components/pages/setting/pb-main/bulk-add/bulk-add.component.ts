...
export class BulkAddComponent implements OnInit, OnDestroy {
  ...
  // キャンセル用モーダルのタイトル
  cancelModalTitle =
    'メイン商品ブック一括登録キャンセル：' + modalConst.TITLE.CANCEL;
  ...
  bulkAddForm = this.formBuilder.group({
  ...
  descriptions = [
    {
      title: 'id',
      description:
        'メイン商品ブックID（半角数字）※空欄にすると新規登録になります。',
    },
    {
      title: 'product_book_second_category_id',
      description: '商品ブック第2分類ID（半角数字）',
      required: true,
    },
    {
      title: 'name',
    ...
    {
      title: 'product_id',
      description: '商品ID（半角数字）',
      required: true,
    },
    {
      title: 'barcode',
      description: '商品JANコード（半角14文字以内）',
    },
    {
      title: 'gyo',
      description: 'プリントコード（半角数字8文字以内）',
      required: true,
    },
    {
      title: 'size_name',
      description: 'サイズ名（10文字以内）',
    },
  ];

  ngOnInit(): void {
    // テンプレートダウンロード用のオブザーバブルを作成
    this.template$ = this.pbMainService.getCsv('template').pipe(
    ...
    this.subscription.add(
      ...
      this.bulkAddForm.controls.file.valueChanges.subscribe((file) => {
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            this.router.navigateByUrl('/main-setting/product-book');
          }
        })
    );
  }
  ...
  limitFileSizeValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (file?.size && file?.size > generalConst.IMAGE_MAX_FILE_SIZE) {
        ...
        this.bulkAddForm.controls.file.setValue(null);
  ...
  handleError(status: number, message: string): void {
    ...
    this.errorService.setError({
      status: status,
      title: 'メイン商品ブック一括登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
  ...
  handleClickUploadButton(): void {
    ...
    this.subscription.add(
      // 商品ブック第1分類一括登録実行
      this.pbMainService
        .bulkAdd(this.bulkAddForm.controls.file.value)
        .pipe(
