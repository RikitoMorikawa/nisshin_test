...
export class ListComponent implements OnInit {
  ...
  tableNameMapping: Mapping<SpecialSale>[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'special_sale_cd', name_jp: '特売コード' },
    { name: 'store_id', name_jp: '対象店舗' },
    { name: 'product_id', name_jp: '対象商品' },
    { name: 'special_sale_price', name_jp: '特売価格' },
    { name: 'start_date', name_jp: '開始日' },
    { name: 'end_date', name_jp: '終了日' },
  ];
  ...
  csvMappings: CsvMapping[] = [
    ...
    { name: 'special_sale_cd', prop_name: 'special_sale_cd' },
    { name: 'store_id', prop_name: 'store_id' },
    { name: 'product_id', prop_name: 'product_id' },
    { name: 'barcode', prop_name: 'barcode' },
    { name: 'start_date', prop_name: 'start_date' },
    { name: 'end_date', prop_name: 'end_date' },
    { name: 'special_sale_price', prop_name: 'special_sale_price' },
    { name: 'special_sale_const_price', prop_name: 'special_sale_const_price' },
  ...
  private callApi() {
    ...
    this.service
      ...
      .subscribe((res) => {
        const params = this.createTableParams<SpecialSale>(
          res,
   ...
   */
  private modifiedTableBody(record: SpecialSale) {
    return (mapping: Mapping<SpecialSale>) => {
      // 値が空なら何もしない
      ...
      // 店舗IDならリンクに修正
      if (mapping.name === 'store_id') {
        return {
          href: `/setting/store/detail/${record[mapping.name]}`,
          text: record.store_name,
        };
      }

      // 商品IDならリンクに修正
      if (mapping.name === 'product_id') {
        return {
          href: `/master/product/detail/${record[mapping.name]}`,
          text: record.product_name,
        };
      }

      // 金額系
      if (mapping.name === 'special_sale_price') {
        const special_sale_price = record[mapping.name];
        //return `${Number(record[mapping.name]).toLocaleString()}`;
        return {
          unit: false,
          align: 'right',
          text: Number(special_sale_price),
        };
      }
