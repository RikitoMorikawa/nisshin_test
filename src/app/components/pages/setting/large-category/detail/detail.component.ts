...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = '大分類詳細：' + modalConst.TITLE.HAS_ERROR;

  // 編集画面へのパスを保持
  editPath!: string;

  // 大分類一覧のパス
  largeCategoryListPath = '/setting/large-category';

  // 中分類の一覧のパス
  mediumCategoryPath = '/setting/medium-category';
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // 大分類詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.largeCategoryListPath);
          }
        })
    );
    ...
    this.editPath = '/setting/large-category/edit/' + this.selectedId;
    ...
    this.subscription.add(
      this.lcService
        ...
        .subscribe((res) => {
          ...
          const lcResInvalid = ApiResponseIsInvalid(res);
          ...
          this.largeCategory = displayData;
          this.add_user =
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = '大分類：' + modalConst.TITLE.DELETE;
              ...
              .subscribe((res) => {
                ...
                if (res instanceof HttpErrorResponse) {
                  ...
                  this.errorService.setError({
                    ...
                    redirectPath: this.largeCategoryListPath,
                  });
                } else {
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '大分類詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.largeCategoryListPath,
    });
  }
