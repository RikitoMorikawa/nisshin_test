...
export class EditComponent implements OnInit, OnDestroy {
  ...
  // キャンセル時のモーダルのタイトル
  cancelModalTitle = '大分類編集キャンセル：' + modalConst.TITLE.CANCEL;
  ...
  lcEditForm = this.formBuilder.group({
    name: [this.lc.name, [Validators.required, Validators.maxLength(10)]],
    short_name: [
      this.lc.short_name,
      [
        Validators.required,
  ...
  ngOnInit(): void {
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          ...
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl('/setting/large-category');
          }
        })
    );
    // パスパラメータからidを取得
    ...
    // 大分類詳細のパスを生成
    this.detailPath = '/setting/large-category/detail/' + this.selectedId;
    ...
    this.subscription.add(
      // フォームの変更を購読
      this.lcEditForm.valueChanges.subscribe((res) => {
    ...
    this.subscription.add(
      this.lcService
        ...
        .subscribe((res) => {
          ...
          // メンバへ取得したデータを代入
          this.lc = res.data[0];
          this.beforeUpdateLc = res.data[0];

          this.lcEditForm.patchValue(this.lc);
        })
    );
  }
  ...
  checkFormDefaultValueChanged(formValues: any): void {
    ...
    const v2 = JSON.stringify({ name: this.beforeUpdateLc.name });
  ...
  handleClickClearButton() {
    this.lcEditForm.reset(this.beforeUpdateLc);
  }
  ...
  handleClickSaveButton() {
    ...
    this.common.loading = true;
    // フォームの値を取得
    const formVal = this.lcEditForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '大分類編集エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
