...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = '区分詳細：' + modalConst.TITLE.HAS_ERROR;

  // 編集画面へのパスを保持
  editPath!: string;

  // 更新者を格納
  last_update_user = '未登録';
  // 登録者
  add_user?: string;

  // 区分一覧のパス
  divisionListPath = '/setting/division';
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // 区分詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.divisionListPath);
          }
        })
    );
    ...
    this.editPath = '/setting/division/edit/' + this.selectedId;
  ...
  getDisplayData() {
    ...
    console.log('Start:' + this.common.loading);
    // 購読を格納
    this.subscription.add(
      this.divisionService
        ...
        .subscribe((res) => {
          ...
          const divisionResInvalid = ApiResponseIsInvalid(res);

          if (divisionResInvalid) {
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = '区分：' + modalConst.TITLE.DELETE;
              ...
              .subscribe((res) => {
                ...
                if (res instanceof HttpErrorResponse) {
                  ...
                  this.errorService.setError({
                    ...
                    redirectPath: this.divisionListPath,
                  });
                } else {
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '区分詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.divisionListPath,
    });
  }
