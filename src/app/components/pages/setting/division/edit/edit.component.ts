...
export class EditComponent implements OnInit, OnDestroy {
  ...
  // キャンセル時のモーダルのタイトル
  cancelModalTitle = '区分編集キャンセル：' + modalConst.TITLE.CANCEL;
  ...
  divisionEditForm = this.formBuilder.group({
    name: [
      this.division.name,
  ...
  ngOnInit(): void {
    ...
    // 区分詳細のパスを生成
    this.detailPath = '/setting/division/detail/' + this.selectedId;
    ...
    this.subscription.add(
      // フォームの変更を購読
      this.divisionEditForm.valueChanges.subscribe((res) => {
    ...
    this.subscription.add(
      this.divisionService
        ...
        .subscribe((res) => {
          ...
          this.beforeUpdateDivision = res.data[0];
          this.divisionEditForm.patchValue(this.division);
          this.last_update_user = `${this.division.employee_updated_last_name} ${this.division.employee_updated_first_name}`;
        })
    );
  }
  ...
  checkFormDefaultValueChanged(formValues: any): void {
    // 初期値から必要な項目のみ抽出
    const defaultValue = {
      name: this.beforeUpdateDivision.name,
      value: this.beforeUpdateDivision.value,
      division_code: this.beforeUpdateDivision.division_code,
    };
    // オブジェクトの比較の為にjsonへ変更
  ...
  handleClickClearButton() {
    this.divisionEditForm.reset(this.beforeUpdateDivision);
  }
  ...
  handleClickSaveButton() {
    ...
    this.common.loading = true;
    // フォームの値を取得
    const formVal = this.divisionEditForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '区分編集エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
