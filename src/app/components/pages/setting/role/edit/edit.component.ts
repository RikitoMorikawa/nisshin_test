...
import { Role, roleLogicalNamesForDisplay } from 'src/app/models/role';
...
export class EditComponent implements OnInit, OnDestroy {
  ...
  // キャンセル時のモーダルのタイトル
  cancelModalTitle = '権限編集キャンセル：' + modalConst.TITLE.CANCEL;
  ...
  roleEditForm = this.formBuilder.group({
  ...
  ngOnInit(): void {
    ...
    // 権限詳細のパスを生成
    this.detailPath = '/setting/role/detail/' + this.selectedId;
    ...
    this.subscription.add(
      // フォームの変更を購読
      this.roleEditForm.valueChanges.subscribe((res) => {
    ...
    this.subscription.add(
      this.roleService
        ...
        .subscribe((res) => {
          ...
          this.beforeUpdateRole = res.data[0];
          this.last_update_user = `${this.role.employee_updated_last_name} ${this.role.employee_updated_first_name}`;
  ...
  checkFormDefaultValueChanged(formValues: any): void {
    // 初期値から必要な項目のみ抽出
    const defaultValue = {
      name: this.beforeUpdateRole.name,
      description: this.beforeUpdateRole.description,
    };
    // オブジェクトの比較の為にjsonへ変更
  ...
  handleClickClearButton() {
    this.roleEditForm.reset(this.beforeUpdateRole);
  }
  ...
  handleClickSaveButton() {
    ...
    this.common.loading = true;
    // フォームの値を取得
    const formVal = this.roleEditForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '権限編集エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
