...
export class CreditAddComponent implements OnInit, OnDestroy {
  ...
  // キャンセル時のモーダルタイトル
  cancelModalTitle = 'クレジット登録キャンセル：' + modalConst.TITLE.CANCEL;
  // フォーム
  creditAddForm = this.formBuilder.group({
  ...
  handleClickCancel() {
    ...
    if (this.creditAddForm.status === 'VALID') {
  ...
  handleClickSaveButton() {
    // 保存ボタンを非活性化
    this.creditAddForm.markAsPristine();
    this.common.loading = true;
    const formValues = this.creditAddForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'クレジット登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
