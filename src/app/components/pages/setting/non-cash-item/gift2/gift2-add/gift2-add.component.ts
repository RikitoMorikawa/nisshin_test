...
@Component({
  selector: 'app-gift2-add',
  templateUrl: './gift2-add.component.html',
  styleUrls: ['./gift2-add.component.scss'],
})
export class Gift2AddComponent implements OnInit {
  /******************** フォームエラーメッセージ用変数 ********************/
  // 制限文字数超過
  ...
  // キャンセル時のモーダルタイトル
  cancelModalTitle = 'ギフト2登録キャンセル：' + modalConst.TITLE.CANCEL;
  // フォーム
  gift2AddForm = this.formBuilder.group({
  ...
  handleClickCancel() {
    ...
    if (this.gift2AddForm.status === 'VALID') {
  ...
  handleClickSaveButton() {
    // 保存ボタンを非活性化
    this.gift2AddForm.markAsPristine();
    this.common.loading = true;
    const formValues = this.gift2AddForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'ギフト2登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
