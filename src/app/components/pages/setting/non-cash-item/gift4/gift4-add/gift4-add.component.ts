...
@Component({
  selector: 'app-gift4-add',
  templateUrl: './gift4-add.component.html',
  styleUrls: ['./gift4-add.component.scss'],
})
export class Gift4AddComponent implements OnInit, OnDestroy {
  /******************** フォームエラーメッセージ用変数 ********************/
  // 制限文字数超過
  ...
  author!: Employee;
  // ギフト4一覧へのパス
  gift4RootPath = '/setting/non-cash-item/gift4';
  // キャンセル時のモーダルタイトル
  cancelModalTitle = 'ギフト4登録キャンセル：' + modalConst.TITLE.CANCEL;
  // フォーム
  gift4AddForm = this.formBuilder.group({
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          ...
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            this.router.navigateByUrl(this.gift4RootPath);
          }
        })
    );
  ...
  handleClickCancel() {
    ...
    if (this.gift4AddForm.status === 'VALID') {
  ...
  handleClickSaveButton() {
    // 保存ボタンを非活性化
    this.gift4AddForm.markAsPristine();
    this.common.loading = true;
    const formValues = this.gift4AddForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'ギフト4登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
