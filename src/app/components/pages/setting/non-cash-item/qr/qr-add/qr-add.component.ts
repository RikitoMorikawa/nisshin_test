...
export class QrAddComponent implements OnInit, OnDestroy {
  /******************** フォームエラーメッセージ用変数 ********************/
  // 制限文字数超過
  ...
  author!: Employee;
  // QR一覧へのパス
  qrRootPath = '/setting/non-cash-item/qr';
  // キャンセル時のモーダルタイトル
  cancelModalTitle = 'QR登録キャンセル：' + modalConst.TITLE.CANCEL;
  // フォーム
  qrAddForm = this.formBuilder.group({
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          ...
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            this.router.navigateByUrl(this.qrRootPath);
          }
        })
    );
  ...
  handleClickCancel() {
    ...
    if (this.qrAddForm.status === 'VALID') {
  ...
  handleClickSaveButton() {
    // 保存ボタンを非活性化
    this.qrAddForm.markAsPristine();
    this.common.loading = true;
    const formValues = this.qrAddForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'QR登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
