...
import { CustomLargeCategoryApiResponse } from 'src/app/models/custom-large-category';
...
export class AddComponent implements OnInit, OnDestroy {
  ...
  // 中分類一覧のパス
  cmcRootPath = '/setting/custom-medium-category';

  // キャンセル時のモーダルのタイトル
  cancelModalTitle = 'カスタム中分類登録キャンセル：' + modalConst.TITLE.CANCEL;

  // リアクティブフォームとバリデーションの設定
  cmcAddForm = this.formBuilder.group({
    custom_large_category_id: ['', [Validators.required]],
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          ...
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.cmcRootPath);
          }
        })
    );
   ...
   * @returns boolean
   */
  apiResponseIsInvalid(arg: CustomLargeCategoryApiResponse): boolean {
  ...
  handleClickCancel() {
    // 編集中の場合はモーダルを表示
    if (this.cmcAddForm.status === 'VALID') {
  ...
  handleClickSaveButton() {
    // 送信ボタンを非活性にする
    this.cmcAddForm.markAsPristine();
    // ローディング開始
    this.common.loading = true;
    // フォームの値を取得
    const formVal = this.cmcAddForm.value;

    // 大分類の値チェック
    const lcIdExists = this.clcOptions.some((clc) => {
      const clcVal = Number(clc.value);
      const storeId = Number(formVal.custom_large_category_id);
      return clcVal === storeId;
    });
    if (!lcIdExists) {
      this.cmcAddForm.controls.custom_large_category_id.setValue(null);
      this.cmcAddForm.markAsPristine();
      // ローディング終了
      this.common.loading = false;
      return;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'カスタム中分類登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
