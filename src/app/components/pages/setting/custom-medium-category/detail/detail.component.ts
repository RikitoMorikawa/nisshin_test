...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = 'カスタム中分類詳細：' + modalConst.TITLE.HAS_ERROR;

  // 編集画面へのパスを保持
  editPath!: string;

  // カスタム中分類一覧のパス
  cmcListPath = '/setting/custom-medium-category';

  // カスタム小分類の一覧のパス
  csmPath = '/setting/custom-small-category';
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // カスタム中分類詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.cmcListPath);
          }
        })
    );
    ...
    this.editPath = '/setting/custom-medium-category/edit/' + this.selectedId;
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = 'カスタム中分類：' + modalConst.TITLE.DELETE;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'カスタム中分類詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.cmcListPath,
    });
  }
