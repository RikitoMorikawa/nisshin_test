...
export class BulkAddComponent implements OnInit, OnDestroy {
  ...
  // キャンセル用モーダルのタイトル
  cancelModalTitle =
    '商品ブック第1分類一括登録キャンセル：' + modalConst.TITLE.CANCEL;
  ...
  fcBulkAddForm = this.formBuilder.group({
  ...
  descriptions = [
    {
      title: 'id',
      description:
        '商品ブック第1分類ID（半角数字）※空欄にすると新規登録になります。',
    },
    {
      title: 'name',
      description: '商品ブック第1分類名（15文字以内）',
      required: true,
    },
    {
      title: 'furi',
      description: '商品ブック第1分類名フリガナ（15文字以内）',
    },
  ];
  ngOnInit(): void {
    // テンプレートダウンロード用のオブザーバブルを作成
    this.template$ = this.pbFirstCategoryService.getCsv('template').pipe(
    ...
    this.subscription.add(
      ...
      this.fcBulkAddForm.controls.file.valueChanges.subscribe((file) => {
  ...
  limitFileSizeValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (file?.size && file?.size > generalConst.IMAGE_MAX_FILE_SIZE) {
        ...
        this.fcBulkAddForm.controls.file.setValue(null);
  ...
  handleError(status: number, message: string): void {
    ...
    this.errorService.setError({
      status: status,
      title: '商品ブック第1分類一括登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
  ...
  handleClickUploadButton(): void {
    ...
    this.subscription.add(
      ...
      this.pbFirstCategoryService
        .bulkAdd(this.fcBulkAddForm.controls.file.value)
        .pipe(
