...
import {
  PbFirstCategory,
  PbFirstCategoryApiResponse,
} from 'src/app/models/pb-first-category';
...
export class TableComponent implements OnInit, OnDestroy {
  ...
  tableNameMapping: { name: keyof PbFirstCategory; name_jp: string }[] = [
    { name: 'id', name_jp: 'ID' },
    { name: 'name', name_jp: '第1分類名' },
    ...
    this.subscription.add(
      this.tableEvent
        .pipe(
          ...
          switchMap((x) =>
            this.pbFirstCategoryService.getAll(this.createApiParams(x))
          ),
  ...
  private createTableParams(
    res: PbFirstCategoryApiResponse
  ): TableWithPaginationParams {
    ...
    body.forEach((row) => {
      ...
      row[column] = {
        href: `/setting/product-book-first-category/detail/${row[column]}`,
        text: row[column] + '',
      };
