...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = '商品ブック第1分類詳細：' + modalConst.TITLE.HAS_ERROR;

  // 編集画面へのパスを保持
  editPath!: string;

  // 商品ブック第1分類一覧のパス
  pbFirstCategoryListPath = '/setting/product-book-first-category';

  // 商品ブック第2分類の一覧のパス
  pbSecondBookCategoryPath = '/setting/product-book-second-category';

  add_user!: string;
  // 更新者を格納
  updater!: string;

  pbFirstCategory!: PbFirstCategory;
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // 商品ブック第1分類詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.pbFirstCategoryListPath);
          }
        })
    );
    ...
    this.editPath =
      '/setting/product-book-first-category/edit/' + this.selectedId;
    ...
    this.subscription.add(
      this.pbFirstCategoryService
        ...
        .subscribe((res) => {
          ...
          this.pbFirstCategory = displayData;
          this.add_user =
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = '商品ブック第1分類：' + modalConst.TITLE.DELETE;
              ...
              .subscribe((res) => {
                ...
                if (res instanceof HttpErrorResponse) {
                  ...
                  this.errorService.setError({
                    ...
                    redirectPath: this.pbFirstCategoryListPath,
                  });
                } else {
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '商品ブック第1分類詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.pbFirstCategoryListPath,
    });
  }
