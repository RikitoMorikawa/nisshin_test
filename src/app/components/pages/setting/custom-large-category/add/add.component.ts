...
export class AddComponent implements OnInit, OnDestroy {
  ...
  // カスタム大分類一覧のパス
  clcRootPath = '/setting/custom-large-category';

  // キャンセル時のモーダルのタイトル
  cancelModalTitle = 'カスタム大分類登録キャンセル：' + modalConst.TITLE.CANCEL;

  // リアクティブフォームとバリデーションの設定
  clcAddForm = this.formBuilder.group({
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          ...
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.clcRootPath);
          }
        })
    );
  ...
  handleClickCancel() {
    // 編集中の場合はモーダルを表示
    if (this.clcAddForm.status === 'VALID') {
  ...
  handleClickSaveButton() {
    // 送信ボタンを非活性にする
    this.clcAddForm.markAsPristine();
    // ローディング開始
    this.common.loading = true;
    // フォームの値を取得
    const formVal = this.clcAddForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'カスタム大分類登録エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
