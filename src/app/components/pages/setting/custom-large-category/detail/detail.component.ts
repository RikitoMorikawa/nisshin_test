...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = 'カスタム大分類詳細：' + modalConst.TITLE.HAS_ERROR;

  // 編集画面へのパスを保持
  editPath!: string;

  // カスタム大分類一覧のパス
  clcListPath = '/setting/custom-large-category';

  // カスタム中分類の一覧のパス
  cmcPath = '/setting/custom-medium-category';
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // カスタム大分類詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.clcListPath);
          }
        })
    );
    ...
    this.editPath = '/setting/custom-large-category/edit/' + this.selectedId;
    ...
    this.subscription.add(
      this.clcService
        ...
        .subscribe((res) => {
          ...
          // 安全に配列へアクセスするためにdataの中身を確認
          const clcResInvalid = ApiResponseIsInvalid(res);
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = 'カスタム大分類：' + modalConst.TITLE.DELETE;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'カスタム大分類詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.clcListPath,
    });
  }
