...
import { CustomLargeCategory } from 'src/app/models/custom-large-category';
...
export class EditComponent implements OnInit, OnDestroy {
  ...
  // キャンセル時のモーダルのタイトル
  cancelModalTitle = 'カスタム大分類編集キャンセル：' + modalConst.TITLE.CANCEL;
  ...
  beforeUpdateClc!: CustomLargeCategory;

  clc: CustomLargeCategory = {
    id: 0,
    name: '',
    created_at: '',
    created_id: 0,
  ...
  clcEditForm = this.formBuilder.group({
    name: [this.clc.name, [Validators.required, Validators.maxLength(255)]],
  });
  ...
  ngOnInit(): void {
      ...
      this.modalService.closeEventObservable$
        ...
        .subscribe((res) => {
          ...
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl('/setting/custom-large-category');
          }
        })
    );
    // パスパラメータからidを取得
    ...
    // 大分類詳細のパスを生成
    this.detailPath =
      '/setting/custom-large-category/detail/' + this.selectedId;
    ...
    this.subscription.add(
      // フォームの変更を購読
      this.clcEditForm.valueChanges.subscribe((res) => {
    ...
    this.subscription.add(
      this.clcService
        ...
        .subscribe((res) => {
          ...
          // メンバへ取得したデータを代入
          this.clc = res.data[0];
          this.beforeUpdateClc = res.data[0];

          this.clcEditForm.patchValue(this.clc);
        })
    );
  }
  ...
  checkFormDefaultValueChanged(formValues: any): void {
    ...
    const v2 = JSON.stringify({ name: this.beforeUpdateClc.name });
  ...
  handleClickClearButton() {
    this.clcEditForm.reset(this.beforeUpdateClc);
  }
  ...
  handleClickSaveButton() {
    ...
    this.common.loading = true;
    // フォームの値を取得
    const formVal = this.clcEditForm.value;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: 'カスタム大分類編集エラー：' + modalConst.TITLE.HAS_ERROR,
      message: message,
    });
  }
