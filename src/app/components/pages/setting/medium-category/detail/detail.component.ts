...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = '中分類詳細：' + modalConst.TITLE.HAS_ERROR;

  // 編集画面へのパスを保持
  editPath!: string;

  // 中分類一覧のパス
  mediumCategoryListPath = '/setting/medium-category';

  // 小分類の一覧のパス
  smallCategoryPath = '/setting/small-category';
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.modalService.closeEventObservable$
        .pipe(
          ...
          ) // 中分類詳細エラーのモーダルじゃなければスルー
        )
        .subscribe((res) => {
          // OKボタンクリックの場合のみ処理を実行
          if (res === modalConst.BUTTON_CLICK_VALUE.OK_BUTTON_CLICK_VALUE) {
            // 一覧画面へ移動する
            this.router.navigateByUrl(this.mediumCategoryListPath);
          }
        })
    );
    ...
    this.editPath = '/setting/medium-category/edit/' + this.selectedId;
    ...
    this.subscription.add(
      this.mcService
        ...
        .subscribe((res) => {
          ...
          this.mediumCategory = displayData;
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = '中分類：' + modalConst.TITLE.DELETE;
              ...
              .subscribe((res) => {
                ...
                if (res instanceof HttpErrorResponse) {
                  ...
                  this.errorService.setError({
                    ...
                    redirectPath: this.mediumCategoryListPath,
                  });
                } else {
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '中分類詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.mediumCategoryListPath,
    });
  }
