...
export class DetailComponent implements OnInit, OnDestroy {
  ...
  // エラーモーダルのタイトル
  errorModalTitle = '在庫移動履歴詳細：' + modalConst.TITLE.HAS_ERROR;
  ...
  // 在庫移動履歴一覧のパス
  listPath = '/inventory-control/stock-transfer';
  ...
  handleClickDelete() {
    // モーダルのタイトル
    const modalTitle = '在庫移動履歴：' + modalConst.TITLE.DELETE;
  ...
  handleError(status: number, message: string) {
    ...
    this.errorService.setError({
      status: status,
      title: '在庫移動履歴詳細：' + modalConst.TITLE.HAS_ERROR,
      message: message,
      redirectPath: this.listPath,
    });
  }
