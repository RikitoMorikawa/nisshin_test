...
export class AddComponent implements OnInit, OnDestroy {
  ...
  // キャンセル時のモーダルのタイトル
  cancelModalTitle =
    'レンタル商品新規登録キャンセル：' + modalConst.TITLE.CANCEL;

  // エラーモーダルのタイトル
  errorModalTitle = 'レンタル商品新規登録エラー：' + modalConst.TITLE.HAS_ERROR;
  ...
  handleClickCalcButton() {
    ...
    if (
      ...
      Number(this.tempSalesTaxDivisionId) === 31
    ) {
      // 外税8%
    ...
    this.subscription.add(
      forkJoin([
        ...
        .subscribe((res) => {
          ...
          /* 全ての区分へ初期値をセット */
          this.fc.sales_tax_division_id.setValue(
            String(this.salesTaxDivisionOptions[0].value)
          );
          this.fc.sales_fraction_division_id.setValue(
            String(this.salesFractionDivisionOptions[0].value)
          );
          this.fc.discount_division_id.setValue(
            String(this.discountDivisionOptions[0].value)
          );
          this.fc.point_division_id.setValue(
            String(this.pointDivisionOptions[0].value)
          );
          this.fc.status_division_id.setValue(
            String(this.statusDivisionOptions[0].value)
          );
          this.fc.product_division_id.setValue(
            String(this.productDivisionOptions[0].value)
          );
          this.fc.data_permission_division_id.setValue(
            String(this.dataPermissionDivisionOptions[0].value)
          );
  ...
  handleClickSaveButton() {
    ...
    const postData: { [key: string]: string | null | undefined } = {
      ...
      delivery_charge_flag: String(formVal.delivery_charge_flag),
