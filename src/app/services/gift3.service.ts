...
import { Gift3ApiResponse } from '../models/gift3';
import { HttpService } from './shared/http.service';

@Injectable({
  providedIn: 'root',
})
export class Gift3Service extends HttpService {
  // クレジットのエンドポイント
  readonly endpoint = protectedResources.gift3Api.endpoint;
   ...
   */
  getAll(params: object = {}): Observable<Gift3ApiResponse> {
    this.httpOptions.params = params;
    return this.http
      .get<Gift3ApiResponse>(this.endpoint, this.httpOptions)
  ...
  find(id: number) {
    return this.http
      .get<Gift3ApiResponse>(`${this.endpoint}/${id}`)
   ...
   * @returns レスポンスのObservable
   */
  add(gift3: any): Observable<Gift3ApiResponse> {
    const formData = this.generateFormData(gift3);
    return this.http
      .post<Gift3ApiResponse>(this.endpoint, formData)
   ...
   * @returns レスポンスのObservable
   */
  update(id: number, gift3: any): Observable<Gift3ApiResponse> {
    const formData = this.generateFormData(gift3);
    return this.http
      .patch<Gift3ApiResponse>(`${this.endpoint}/${id}`, formData)
   ...
   * @returns レスポンスのObservable
   */
  remove(id: number): Observable<Gift3ApiResponse> {
    return this.http
      .delete<Gift3ApiResponse>(`${this.endpoint}/${id}`)
